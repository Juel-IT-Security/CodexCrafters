...
> **Adding a new agent?**  Fork this table, pick a concise **upper‑snake‑case id**, document the responsibilities and deliverables, and reference the change in the **Change Log** (section 11).
...
When planning work, Codex outputs checklists grouped by agent:

```markdown
### Task List [AGENT_ID]

- [ ] Short imperative task — _optional/file/path.ts_
- [ ] ...
```

* One section per agent.
* Keep tasks concise (≤ 100 chars).
* If multiple agents collaborate, include multiple `### Task List` blocks.

---

## 4. Commit-Message Rules

* Begin commit messages with **type(scope):** (`feat(api): ...`).
* Limit the summary line to 72 characters.
* Wrap body lines at 100 characters.
* Reference issues or ADRs in the footer (`Refs #42`, `ADR-007`).

---

## 5. Memory & Context Windows
